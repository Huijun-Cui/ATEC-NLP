...
class DecomseAtten(nn.Module):
    ...
    def __init__(self, num_embeddings_c, embedding_size_c, num_embeddings_w, embedding_size_w, hidden_size_c, \
        ...
        self.final_linear = self._mlp_final_layers(4 * self.hidden_size_c + 4 * self.hidden_size_w, 2)
        self.label_size = label_size
    ...
    def forward(self, sent1_c, sent2_c, sent1_w, sent2_w, feat):
        ...
        '''
        # ---------------------------------------------------------------------------------
        # feat_vec = self.feat_liear(feat)
        ba_size = sent1_c.size()[0]
        ...
        h = torch.cat([sent1_output_c, sent2_output_c, sent1_output_w, sent2_output_w], dim=1)
...
def eval(model, df_c, df_w, w2i_c, w2i_w, maxlen_c, maxlen_w, model_char, model_word,model_char_2gram,\
    ...
    for ix in ix_batch:
        ...
        output = model(sen1c, sen2c, sen1w, sen2w , feat = None)
        ...
        output_soft = torch.nn.functional(output,dim = 1)
...
def train():
    ...
    for e in range(epoch):
        ...
        for ix in ix_batch:
            ...
            optimizer.zero_grad()
            output = model(sen2c, sen1c, sen2w, sen1w, feat = None)
            loss = criterion(output, label)
            loss.backward()
            optimizer.step()
