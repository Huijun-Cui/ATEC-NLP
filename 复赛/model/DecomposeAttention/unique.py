...
class DecomseAtten(nn.Module):
    ...
    def __init__(self, num_embeddings_c, embedding_size_c, num_embeddings_w, embedding_size_w, hidden_size_c, \
        ...
        self.final_linear = self._mlp_final_layers(4 * self.hidden_size_c + 4 * self.hidden_size_w + 30*4, 2)
        self.label_size = label_size
    ...
    def forward(self, sent1_c, sent2_c, sent1_w, sent2_w, uni_1,uni_2,uni_1_w,uni_2_w):
        ...
        h = torch.cat([sent1_output_c, sent2_output_c, sent1_output_w,\
                       sent2_output_w,c_iter,w_iter,uni_iter,uni_iter_w], dim=1)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(FILE_PATH):
        os.mkdir(FILE_PATH)
    df1 = pd.read_csv('train-c.csv', sep='\t', names=['id', 'sent1', 'sent2', 'label'], encoding='utf-8').head(350)
    df2 = pd.read_csv('train-w.csv', sep='\t', names=['id', 'sent1', 'sent2', 'label'], encoding='utf-8').head(350)
