...
class DRCN(nn.Module):
    def __init__(self,opt):
        ...
        self.scale_cnn = nn.Conv2d(in_channels=self.hidden_size * 2,\
                                   out_channels=self.hidden_size,kernel_size=(1,1))
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2):
        ...
        #  B * len2 * hidden_size
        score1 = torch.bmm(sen1_atten, torch.transpose(sen1_atten, 1, 2))
        score2 = torch.bmm(sen2_atten, torch.transpose(sen2_atten, 1, 2))
        # B * len1 * len2
        ...
        sent1_combine = torch.cat(
            (sen1_atten, torch.bmm(prob1, sen1_atten)), 2)
        # batch_size x len1 x (hidden_size x 2)
        sent2_combine = torch.cat(
            (sen2_atten, torch.bmm(prob2, sen2_atten)), 2)
        ...
        feat_extrac = F.avg_pool2d(feat_extrac, kernel_size=7).view(feat_extrac.size(0), -1)
