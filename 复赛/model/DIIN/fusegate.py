...
class DRCN(nn.Module):
    def __init__(self,opt):
        ...
        self.scale_cnn = nn.Conv2d(in_channels=self.hidden_size,\
                                   out_channels= 3,kernel_size=(1,1))
        self.dense_net = nn.Sequential(*list(models.densenet121().children())[:-1])

        self.W = nn.Linear(4 * self.hidden_size,6 * self.hidden_size)
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2):
        ...
        sen_combine = torch.cat([sent1_combine,sent2_combine],dim = 2)

        sen_W = self.W(sen_combine)

        z_w1,r_w1,f_w1,z_w2,r_w2,f_w2 = torch.split(sen_W,self.hidden_size,dim = 2)

        z1 = nn.functional.tanh(z_w1)
        r1 = nn.functional.sigmoid(r_w1)
        f1 = nn.functional.sigmoid(f_w1)

        P = sen1_atten * r1 + f1 * z1

        z2 = nn.functional.tanh(z_w2)
        r2 = nn.functional.sigmoid(r_w2)
        f2 = nn.functional.sigmoid(f_w2)

        H = sen2_atten * r2 + f2 * z2
        ...
        sent1_expand = P.unsqueeze(2)
        sent2_expand = H.unsqueeze(1)
