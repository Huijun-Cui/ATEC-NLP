...
class ESIM(nn.Module):
    def __init__(self,opt):
        ...
        self.lstm_2nd = nn.LSTM(input_size= 4 * self.hidden_size,\
                                hidden_size=self.hidden_size,bidirectional=True)
        ...
        # self.pooling_linear = nn.Linear(6* self.hidden_size + 2* opt.embedding_dims + 2,opt.embedding_dims)

        self.fc = nn.Linear(8 * self.hidden_size,2)
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2):
        ...
        sent1_align = torch.bmm(prob1, sen2_atten)
        sent2_align = torch.bmm(prob2, sen1_atten)
        sent1_sub = sen1_atten - sent1_align
        sent1_mult = sen1_atten * sent1_align

        sent1_combine = torch.cat(
            (sen1_atten, sent1_align,sent1_sub,sent1_mult), 2).permute(1,0,2)

        # -----------------------------------------------------------------
        sent2_sub = sen2_atten - sent1_align
        sent2_mult = sen2_atten * sent2_align

        sent2_combine = torch.cat(
            (sen2_atten,sent2_align,sent2_sub,sent2_mult),2
        ).permute(1,0,2)
        ...
        sen1_pooling_2layer = torch.cat([lstm_output_sen1_2layer.mean(dim=1),\
                                         lstm_output_sen1_2layer.max(dim=1)[0]], dim=1)
        sen2_pooling_2layer = torch.cat([lstm_output_sen2_2layer.mean(dim=1), \
                                         lstm_output_sen2_2layer.max(dim=1)[0]], dim=1)

        final_vec = torch.cat([sen1_pooling_2layer,sen2_pooling_2layer],dim = 1)
...
def train():
    ...
    for e in range(epoch):
        ...
        count = 0
        for sen1c,sen2c,len1c,len2c,comsen1c,comsen2c,label in Data_batch:
            count += 1
            optimizer.zero_grad()


            output = model(sen1c, sen2c, len1c,len2c, comsen1c, comsen2c)
            # import ipdb
            # ipdb.set_trace()
