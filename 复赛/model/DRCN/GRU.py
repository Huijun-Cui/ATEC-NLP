...
class DRCN(nn.Module):
    def __init__(self,opt,num_embeddings,embedding_size,hidden_size, \
        ...
        self.lstm = nn.GRU(input_size=self.embedding_size + 1,\
                            hidden_size=self.hidden_size,bidirectional=True)
        self.lstm_2nd = nn.GRU(input_size= 2 * self.hidden_size + self.embedding_size + 1,\
                                hidden_size=self.hidden_size,bidirectional=True)
        ...
        self.pooling_linear = nn.Linear(4* self.hidden_size + 2* embedding_size + 2,self.embedding_size)
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2,feat = None):
        ...
        lstm_output_packed_sen1, _ = self.lstm(pack_sen1, ini_hidden)
        ...
        ini_hidden, ini_cell = self.initial(batch_size=batch_size)
        lstm_output_packed_sen2, _ = self.lstm(pack_sen2, ini_hidden)
        ...
        lstm_output_packed_sen1,_ = self.lstm_2nd(pack_sen1_layer2,ini_hidden2layer)
        ...
        ini_hidden2layer, ini_cell2layer = self.initial2layer(batch_size=batch_size)
        lstm_output_packed_sen2, _ = self.lstm_2nd(pack_sen2_layer2,ini_hidden2layer)
...
if __name__ == '__main__':
    batch_size = 256
    model_dir = 'model_dir'
    FILE_PATH = model_dir + '/DRCN'
    df1 = pd.read_csv('dfc.csv', sep='\t',\
                      names=['id', 'sent1', 'sent2', 'label'],\
                      encoding='utf-8').head(35000)
    df2 = pd.read_csv('dfw.csv', sep='\t', \
                      names=['id', 'sent1', 'sent2', 'label'],\
                      encoding='utf-8').head(35000)
