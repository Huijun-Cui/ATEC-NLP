...
class Config(object):
    def __init__(self):
        self.embedding_dims_word = 70
        self.hidden_dims_word = 60
        self.embedding_dims_char = 60
        self.hidden_dims_char = 70
...
class DRCN(nn.Module):
    def __init__(self,opt,num_embeddings,embedding_size,hidden_size, \
                 comman_size,label_size = 2,feat_size =30,weight = None):
        ...
        self.lstm_2nd = nn.LSTM(input_size= 3 * self.hidden_size + self.embedding_size + 1,\
                                hidden_size=self.hidden_size,bidirectional=True)
        ...
        self.pooling_linear = nn.Linear(6* self.hidden_size + 2* embedding_size + 2,self.embedding_size)

        self.fc = nn.Linear(8 * self.embedding_size,2)
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2,feat = None):
        ...
        lstm_output_sen2 = self.unpack_from_rnn_seq(lstm_output_packed_sen1,\
        ...
        sent1_combine = torch.cat(
            (sen1_atten, torch.bmm(prob1, sen2_atten),torch.bmm(prob2, sen2_atten)), 2)
        # batch_size x len1 x (hidden_size x 3)
        sent2_combine = torch.cat(
            (sen2_atten, torch.bmm(prob2, sen1_atten),torch.bmm(prob1, sen1_atten)), 2)
        # batch_size x len2 x (hidden_size x 3)
        ...
        #  len1 x batch_size x (hidden_size x 3 + emb_size + 1)
        sent2_combine = torch.cat([sent2_combine, sen2_emb],dim= 2)
        #  len2 x batch_size x (hidden_size x 3 + emb_size + 1)
        ...
        # -----------------------------------------------------------------------------------------------
        sen1_atten_2layer = self.projection(lstm_output_sen1_2layer)
        #  B * len1 * hidden_size
        sen2_atten_2layer = self.projection(lstm_output_sen2_2layer)
        #  B * len2 * hidden_size
        ...
        sent1_combine_2layer = torch.cat(
            (sen1_atten_2layer, torch.bmm(prob1_2layer, sen2_atten_2layer),\
             torch.bmm(prob2_2layer, sen2_atten_2layer)), 2)
        # batch_size x len1 x (hidden_size x 3)
        sent2_combine_2layer = torch.cat(
            (sen2_atten_2layer, torch.bmm(prob2_2layer, sen1_atten_2layer),\
             torch.bmm(prob1_2layer, sen1_atten_2layer)), 2)
        # batch_size x len2 x (hidden_size x 3)

        sent1_combine_2layer = torch.cat([sent1_combine_2layer,\
                                          sent1_combine.permute(1,0,2),sen1_emb.permute(1,0,2)],dim = 2)
        # batc_size * len1 * (6 * hideen_size + 2 * emb_size + 2)
        sent2_combine_2layer = torch.cat([sent2_combine_2layer,\
                                          sent2_combine.permute(1,0,2),sen2_emb.permute(1,0,2)],dim = 2)

        # batc_size * len2 * (6 * hideen_size + 2 * emb_size + 2)
        ...
        final_vec = torch.cat([sen1_pooling_2layer,sen2_pooling_2layer,sen1_pooling_2layer+sen2_pooling_2layer,\
                               torch.abs(sen1_pooling_2layer - sen2_pooling_2layer)],dim = 1)
...
def CreateBatchTensorWord(w2i_w, maxlen_w, df):
    ...
    for sen1, sen2 in zip(df['sent1'], df['sent2']):
        ...
        sen1_drop = re.sub(u'[^\u4e00-\u9fa5A-Za-z0-9*\s]', "", sen1_drop)
        # sen2_drop = pattern.sub(r'', sen2)

        sen2_drop = sen2.replace('***', '*')
        sen2_drop = re.sub(u'[^\u4e00-\u9fa5A-Za-z0-9*\s]', "", sen2_drop)
        # sen1_drop = pattern.sub(r'', sen1)
        # sen2_drop = pattern.sub(r'', sen2)
        ...
        # ------------------------------------------------------------------------------------------------
        for ix,word1 in enumerate(sen1_list):
            if word1 in sen2_list and word1 != 0:
                com1_tmp_list[ix] = 1
        for ix,word in enumerate(sen2_list):
            if word in sen1_list  and word != 0:
                com2_tmp_list[ix] = 1
        com_sen1.append(com1_tmp_list)
        com_sen2.append(com2_tmp_list)
    ...
    return torch.LongTensor(result_sen1).cuda(), torch.LongTensor(result_sen2).cuda(), \
           torch.tensor(result_len1).cuda(),\
           torch.tensor(result_len2).cuda(),torch.FloatTensor(com_sen1).cuda().unsqueeze(2),\
           torch.FloatTensor(com_sen2).cuda().unsqueeze(2)
...
def eval(model,df_c,df_w,w2i_c,w2i_w,maxlen_c,maxlen_w,model_char,model_word, \
         model_char_2gram,tfidf_dict_c,tfidf_dict_w,tfidf_dict_bigram):
    with torch.no_grad():
        model.eval()
        y_p_float = []
        y_p_int = []
...
def train():
    ...
    model = DRCN(opt,num_embeddings= len(i2w_c),embedding_size= opt.embedding_dims_char,\
                 hidden_size=opt.hidden_dims_char,comman_size=maxlen_c,weight=weights_c).cuda()
    ...
    for e in range(epoch):
        ...
        for ix in ix_batch:
            ...
            # sen1w, sen2w, len1w, len2w,comsen2w,comsen2w = CreateBatchTensorWord(w2i_w, maxlen_w, train_w.iloc[ix.numpy().tolist()])
            ...
            # -----------------------------------------------------------------------------------------------
            optimizer.zero_grad()
            output = model(sen2c, sen1c, len2c, len1c, comsen2c, comsen1c, feat=None)
            loss = criterion(output, label)
            loss.backward()
            optimizer.step()
        ...
        df_loss, f1, precision_rate, recall_rate = eval(model, valid_c, \
                                                        ...
                                                        tfidf_dict_c, tfidf_dict_w,tfidf_dict_bigram)
        print('the best f1 is{}'.format(f1))
...
if __name__ == '__main__':
    batch_size = 256
    # model_dir = 'model_dir'
    FILE_PATH = model_dir + '/DRCNContrastiveAtten'
