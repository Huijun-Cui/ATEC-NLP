...
class DRCN(nn.Module):
    ...
    def forward(self,sen1,sen2,len1,len2,comman_sen1,comman_sen2,feat = None):
        ...
        #  len1 x batch_size x (hidden_size x 4)
        sent2_combine = sent2_combine.permute(1, 0, 2)
        #  len2 x batch_size x (hidden_size x 4)

        sent1_combine = torch.cat([sent1_combine,sen1_emb],dim = 2)
        #  len1 x batch_size x (hidden_size x 4 + emb_size + 1)
        ...
        # -----------------------------------------------------------------------------------------------
        sen1_atten_2layer = F.normalize(lstm_output_sen1_2layer,dim = 2)
        sen2_atten_2layer = F.normalize(lstm_output_sen2_2layer,dim = 2)
        ...
        sent1_combine_2layer = torch.cat(
            (lstm_output_sen1_2layer, torch.bmm(prob1_2layer, lstm_output_sen2_2layer)), 2)
        # batch_size x len1 x (hidden_size x 4)
        sent2_combine_2layer = torch.cat(
            (lstm_output_sen2_2layer, torch.bmm(prob2_2layer, lstm_output_sen1_2layer)), 2)
        # batch_size x len2 x (hidden_size x 4)
