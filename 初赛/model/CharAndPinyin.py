...
class CharAndPin(nn.Module):
    ...
    def initial(self,name,batch_size):
        if name == 'word_2layer_lstm':
            return Variable(torch.randn(2 * 1,batch_size,self.opt.hidden_dims_word)), \
                   Variable(torch.randn(2 * 1, batch_size, self.opt.hidden_dims_word))
        if name == 'word_bi_lstm':
            return Variable(torch.randn(2*  1,batch_size,self.opt.hidden_dims_word)), \
                   Variable(torch.randn(2 * 1, batch_size, self.opt.hidden_dims_word))
        if name == 'char_bi_lstm':
            return Variable(torch.randn(2 * 1,batch_size,self.opt.hidden_dims_char)), \
                   Variable(torch.randn(2 * 1, batch_size, self.opt.hidden_dims_char))
